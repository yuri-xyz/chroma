...
impl AudioCapture {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let device = host
            ...
            .ok_or("No input device available. Check system audio settings.")?;
        ...
        let stream = match config.sample_format() {
            cpal::SampleFormat::F32 => {
                Self::build_stream::<f32>(&device, &config.into(), buffer_clone)?
            }
            cpal::SampleFormat::I16 => {
                Self::build_stream::<i16>(&device, &config.into(), buffer_clone)?
            }
            cpal::SampleFormat::U16 => {
                Self::build_stream::<u16>(&device, &config.into(), buffer_clone)?
            }
