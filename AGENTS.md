...
- Third-party GitHub Actions in `.github/workflows/` are pinned to a full commit SHA with the version in a trailing comment, never to a branch or a moving tag such as `@master` or `@v4`. Look the SHA up from the upstream repository when updating one. Jobs run on named runner images (`ubuntu-24.04`, `windows-2025`), not `ubuntu-latest` or `windows-latest`, and any tool downloaded in a step is verified with `sha256sum --check` against the checksum published with that release.
